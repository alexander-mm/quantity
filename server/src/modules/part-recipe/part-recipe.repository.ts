    ...
    async upsert(
        ...
        return this.prisma.partRecipe.upsert({
            ...
            update: {
                ...
                pieceHeight: data.pieceHeight ?? null,
                pieceLength: data.pieceLength ?? null,
                // Si no se manda (lo carga "Recetas de corte", que ya no pide este campo),
                // se deja el valor existente tal cual en vez de pisarlo con null.
                ...(data.piecesPerUnit !== undefined ? { piecesPerUnit: data.piecesPerUnit } : {}),
                laserMeters: data.laserMeters ?? null,
                usesMechanicalCut: data.usesMechanicalCut ?? false,
                bendCount: data.bendCount ?? null,
                curveCount: data.curveCount ?? null
